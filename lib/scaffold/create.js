...
var BASE_PACKAGE = {
  description: 'A full Raptoreum node build with rtmcore',
  repository: 'https://github.com/Raptor3um/rtmcore',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
...
/**
 * Will create a directory and raptor.conf file for Raptoreum.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createRaptoreumDirectory(datadir, done) {
...
/**
 * Will create a base rtmcore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The raptoreum database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/rtmcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a rtmcore Node directory, configuration file,
 * raptoreum configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the rtmcore node configuration directory
 * @param {String} options.datadir - The path to the raptoreum datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the rtmcore-node directory and configuration
    ...
    function(next) {
      // Setup the raptoreum directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createRaptoreumDirectory(absDataDir, next);
      } else {
        next();
      }
    },
