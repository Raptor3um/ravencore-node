...
/**
 * Will return the path and default rtmcore-node configuration. It will search for the
 * configuration file in the "~/.rtmcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.rtmcore');
  var defaultConfigFile = path.resolve(defaultPath, './rtmcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['raptoreumd', 'web'];
