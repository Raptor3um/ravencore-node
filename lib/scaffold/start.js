...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for raptoreumd is necessary for this upgrade with the "reindex=1" raptor.conf option. \n' +
                 'There are changes necessary in both raptor.conf and rtmcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/Raptor3um/rtmcore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed rtmcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './rtmcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in rtmcore-node services directory
    ...
    if (servicePackage.rtmcoreNode) {
      serviceModule = service.name + '/' + servicePackage.rtmcoreNode;
    }
 ...
 * @param {Node} node - The rtmcore Node instance
 */
