...
/**
 * Will remove a service from rtmcore-node.json
...
/**
 * Will remove the Node.js service and from the rtmcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The rtmcore-node configuration directory
...
function remove(options, done) {
  ...
  var rtmcoreConfigPath = path.resolve(configPath, 'rtmcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(rtmcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a rtmcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to rtmcore-node.json
        removeConfig(rtmcoreConfigPath, service, next);
      });
    }, done
  );
}
