...
/**
 * Provides a friendly event driven API to raptoreumd in Node.js. Manages starting and
 * stopping raptoreumd as a child process for application support, as well as connecting
 * to multiple raptoreumd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Raptoreum(options) {
  if (!(this instanceof Raptoreum)) {
    return new Raptoreum(options);
  }
...
util.inherits(Raptoreum, Service);

Raptoreum.dependencies = [];

Raptoreum.DEFAULT_MAX_TXIDS = 1000;
Raptoreum.DEFAULT_MAX_HISTORY = 50;
Raptoreum.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Raptoreum.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Raptoreum.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Raptoreum.DEFAULT_SPAWN_RESTART_TIME = 5000;
Raptoreum.DEFAULT_SPAWN_STOP_TIME = 10000;
Raptoreum.DEFAULT_TRY_ALL_INTERVAL = 1000;
Raptoreum.DEFAULT_REINDEX_INTERVAL = 10000;
Raptoreum.DEFAULT_START_RETRY_INTERVAL = 5000;
Raptoreum.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Raptoreum.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Raptoreum.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Raptoreum.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Raptoreum.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Raptoreum.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Raptoreum.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Raptoreum.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Raptoreum.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Raptoreum.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Raptoreum.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Raptoreum.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Raptoreum.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Raptoreum.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Raptoreum.prototype._initCaches = function() {
...
Raptoreum.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Raptoreum.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Raptoreum.prototype.getPublishEvents = function() {
  return [
    {
...
Raptoreum.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'raptoreumd/' + name, 'total:', this.subscriptions[name].length);
};

Raptoreum.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'raptoreumd/' + name, 'total:', this.subscriptions[name].length);
};

Raptoreum.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    for(var i = 0; i < addresses.length; i++) {
        if (rtmcore.Address.isValid(addresses[i], this.node.network)) {
            addAddress(addresses[i]);
        }
    }

    log.info(emitter.remoteAddress, 'subscribe:', 'raptoreumd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Raptoreum.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'raptoreumd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Raptoreum.prototype.unsubscribeBalanceAll = function(emitter) {
...
Raptoreum.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'raptoreumd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Raptoreum.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'raptoreumd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Raptoreum.prototype.unsubscribeAddressAll = function(emitter) {
...
Raptoreum.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Raptoreum.DEFAULT_CONFIG_SETTINGS;
...
Raptoreum.prototype._parseRaptoreumConf = function(configPath) {
  var options = {};
...
Raptoreum.prototype._expandRelativeDatadir = function() {
...
Raptoreum.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in raptoreumd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in raptoreumd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in raptoreumd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './raptor.conf');

  log.info('Using raptoreum config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseRaptoreumConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseRaptoreumConf(networkConfigPath));
  }
...
Raptoreum.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of rtmcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of rtmcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of rtmcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to raptoreumd from rtmcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from raptoreumd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from raptoreumd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in raptor.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that raptoreumd is undergoing a reindex. ' +
             ...
             'of rtmcore-node services will start.');
    node._reindex = true;
  }
};

Raptoreum.prototype._resetCaches = function() {
...
Raptoreum.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Raptoreum.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Raptoreum.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Raptoreum Daemon Ready');
          callback();
        });
      });
...
Raptoreum.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
  if (this.node.network === rtmcore.Networks.testnet) {
    networkOptions.rpcport = 18766;
  }
  return networkOptions;
};

Raptoreum.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === rtmcore.Networks.testnet) {
    networkPath = 'testnet3/raptor.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/raptor.conf';
    }
  }
  return networkPath;
};

Raptoreum.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === rtmcore.Networks.testnet) {
...
Raptoreum.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('raptoreumd/hashblock', message.toString('hex'));
    }
  }

};

Raptoreum.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Raptoreum.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Raptoreum Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Raptoreum.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Raptoreum.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Raptoreum.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('raptoreumd/addressbalance', {
                    address: address,
                    txid: txid,
...
Raptoreum.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('raptoreumd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Raptoreum.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = rtmcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('raptoreumd/rawtransaction', message.toString('hex'));
    }
...
Raptoreum.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Raptoreum.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Raptoreum.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Raptoreum.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Raptoreum.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Raptoreum Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Raptoreum.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Raptoreum.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Raptoreum.prototype._stopSpawnedRaptoreum = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/raptoreumd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned raptoreum process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean raptoreum process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Raptoreum.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedRaptoreum(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting raptoreum process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Raptoreum process unexpectedly exited with code:', code);
        log.warn('Restarting raptoreum child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Raptoreum process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn raptoreumd.'));
      }
...
Raptoreum.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to raptoreumd.'));
    }
 ...
 */
Raptoreum.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Raptoreum configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.syncPercentage = function(callback) {
  var self = this;
...
Raptoreum.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, rtmcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Raptoreum.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = rtmcore.Script.fromAddress(delta.address);
    return {
...
Raptoreum.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Raptoreum.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Raptoreum.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in rtmcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Raptoreum.prototype._getConfirmationsDetail = function(transaction) {
...
Raptoreum.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Raptoreum.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Raptoreum.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A raptoreum transaction id
 * @param {Function} callback
 */
Raptoreum.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Raptoreum.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof rtmcore.Address) {
...
Raptoreum.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
Raptoreum.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Raptoreum.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getRawBlock = function(blockArg, callback) {
...
Raptoreum.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a rtmcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Raptoreum.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = rtmcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Raptoreum.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
Raptoreum.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a rtmcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Raptoreum.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.getInfo = function(callback) {
  var self = this;
...
Raptoreum.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('raptoreumd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('raptoreumd process did not exit'));
      }
...
Raptoreum.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
Raptoreum.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
Raptoreum.prototype.getMiningInfo = function(callback) {
    var self = this;
...
Raptoreum.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
Raptoreum.prototype.getPeerInfo = function(callback) {
    var self = this;
